...
###
    ...
    public function loadToolConsumer($consumer)
    {
        ...
        $rsConsumer = mysqli_query($this->db, $sql);
        if ($rsConsumer) {
            while ($row = mysqli_fetch_object($rsConsumer)) {
    ...
    public function saveToolConsumer($consumer)
    {
        ...
        if (empty($id)) {
            $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . static::CONSUMER_TABLE_NAME . ' (consumer_key256, consumer_key, name, ' .
                ...
                'enable_from, enable_until, last_access, created, updated) ' .
                'VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %d, %s, %s, %s, %s, %s)', $this->escape($key256),
        ...
        if ($ok) {
            if (empty($id)) {
                $consumer->setRecordId(mysqli_insert_id($this->db));
...
// Delete consumer
    ...
    public function getToolConsumers()
    {
        ...
        $sql = 'SELECT consumer_pk, consumer_key256, consumer_key, name, secret, lti_version, ' .
            ...
            "FROM {$this->dbTableNamePrefix}" . static::CONSUMER_TABLE_NAME . ' ' .
            'ORDER BY name';
        $rsConsumers = mysqli_query($this->db, $sql);
        if ($rsConsumers) {
...
###
    ...
    public function loadContext($context)
    {
        ...
        $rs_context = mysqli_query($this->db, $sql);
        if ($rs_context) {
            $row = mysqli_fetch_object($rs_context);
            if ($row) {
...
###
    ...
    public function loadResourceLink($resourceLink)
    {
        ...
        $rsContext = mysqli_query($this->db, $sql);
        if ($rsContext) {
...
// Delete resource link
    ...
    public function getUserResultSourcedIDsResourceLink($resourceLink, $localOnly, $idScope)
    {
        ...
        $rsUser = mysqli_query($this->db, $sql);
        if ($rsUser) {
    ...
    public function getSharesResourceLink($resourceLink)
    {
        ...
        $rsShare = mysqli_query($this->db, $sql);
        if ($rsShare) {
...
// Load the nonce
        ...
        $rs_nonce = mysqli_query($this->db, $sql);
        if ($rs_nonce) {
            if (mysqli_fetch_object($rs_nonce)) {
                $ok = true;
            }
        }
...
// Load share key
        ...
        $rsShareKey = mysqli_query($this->db, $sql);
        if ($rsShareKey) {
...
###
    ...
    public function loadUserResult($userresult)
    {
        ...
        $rsUser = mysqli_query($this->db, $sql);
        if ($rsUser) {
            $row = mysqli_fetch_object($rsUser);
            if ($row) {
