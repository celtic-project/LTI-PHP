            ...
            while ($row = sqlsrv_fetch_object($rsConsumer)) {
                if (empty($key256) || empty($row->consumer_key) || ($consumer->getKey() === $row->consumer_key)) {
                    ...
                    if (!is_null($row->enable_from)) {
                        $consumer->enableFrom = date_timestamp_get($row->enable_from);
                    }
                    $consumer->enableUntil = null;
                    if (!is_null($row->enable_until)) {
                        $consumer->enableUntil = date_timestamp_get($row->enable_until);
                    }
                    $consumer->lastAccess = null;
                    if (!is_null($row->last_access)) {
                        $consumer->lastAccess = date_timestamp_get($row->last_access);
                    }
                    $consumer->created = date_timestamp_get($row->created);
                    $consumer->updated = date_timestamp_get($row->updated);
                    $ok = true;
                    break;
                }
            }
            sqlsrv_free_stmt($rsConsumer);
        }
