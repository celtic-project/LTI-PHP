...
/**
 * The HMAC-SHA512 signature method uses the HMAC-SHA512 signature algorithm as defined in [RFC6234]
 * where the Signature Base String is the text and the key is the concatenated values (each first
 * encoded per Parameter Encoding) of the Consumer Secret and Token Secret, separated by an '&'
 * character (ASCII code 38) even if empty.
 */
class OAuthSignatureMethod_HMAC_SHA512 extends OAuthSignatureMethod
{
    ...
    public function build_signature(OAuthRequest $request, OAuthConsumer $consumer, ?OAuthToken $token): string
    {
        ...
        return base64_encode(hash_hmac('sha512', $base_string, $key, true));
    }
